...
require __DIR__ .'/multicolor_name.php';
...
foreach ($picks as $pick) {
   ...
   if (isset($card->colors)) {
      ...
      if (count($colors) > 1) {
         $multicolored_count++;
         $color_table[$ndx] = getMulticoloredName($colors);
      }
   ...
   if (isset($card->colorIdentity)) {
      ...
      if (!in_array('Land', $card->types)) { // exclude land picks
         $color_table[$ndx++] = 'Colorless';
      } else {
         $color_table[$ndx++] = 'Land';
      }
   }
...
$data = array();
$data['total_card_count'] = $total_card_count;
$data['multicolored_picks'] = $multicolored_count;
$data['color_count'] = $color_count;
$data['cmc_count'] = $cmc_count;
$data['type_count'] = $type_count;
$data['date'] = date("l h:ia");
$data['color_identities'] = $color_table;

$fp = fopen('peasant_cube_stats.json', 'w');
